    ...
    return new Promise((resolve, reject) => {
      ...
      s.onload = () => {
        ...
        if (window.HeyGenPlayer) {
          class HeygenPlayerCE extends HTMLElement {
            connectedCallback(){
              // Attach shadow DOM
              if (!this.shadowRoot) this.attachShadow({mode:'open'});
              this.shadowRoot.innerHTML = `<div id="player" style="width:100%;height:100%;background:#000;color:#fff">🎬 HeyGen Player Ready</div>`;
              // Call into UMD API
              try {
                window.HeyGenPlayer.create({ element: this.shadowRoot.querySelector('#player') });
                console.log('[heygen-loader2] HeyGen player created inside CE');
              } catch(e){
                console.error('[heygen-loader2] create failed:', e);
              }
            }
          }
          if (!customElements.get('heygen-player2')) {
            customElements.define('heygen-player2', HeygenPlayerCE);
